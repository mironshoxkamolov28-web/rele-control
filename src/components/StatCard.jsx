export function StatCard({ label, value, gradient, icon, delay, onClick }) {
  ...
  return (
    <Tag onClick={onClick}
      className={`group relative overflow-hidden rounded-2xl ${gradient} p-3 sm:p-5 ring-1 ring-white/10 transition-all duration-200 hover:ring-white/20 hover:scale-[1.02] hover:shadow-2xl animate-slide-up w-full text-left ${isClickable ? 'cursor-pointer active:scale-[0.98]' : ''}`}
